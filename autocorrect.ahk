...
{ ; V1toV2: Added bracket
    ...
    hs := StrReplace(A_Clipboard, "`, ```, All",,,, 1)  ; Do this replacement first to avoid interfering with the others below.
    ; StrReplace() is not case sensitive
    ; check for StringCaseSense in v1 source script
    ; and change the CaseSense param in StrReplace() if necessary
    ...
    hs := InputBox("Provide the corrected word on the right side. You can also edit the left side if you wish.`n`nExample entry:`n::teh::the", "New Hotstring", , "::%hs%::%hs%").Value

    if (ErrorLevel != 0)  ; The user pressed Cancel.
        return
    ; Otherwise, add the hotstring and reload the script:
    FileAppend("`n" hs, A_ScriptFullPath)  ; Put a `n at the beginning in case file lacks a blank line at its end.
    Reload()
